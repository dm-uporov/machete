...
internal class MacheteBuilder(
    private val appFeature: Feature
) {
    ...
    fun buildMachete(): FileSpec {
        return FileSpec.builder(coreClassPackage, "Machete")
            .addType(
    ...
    private fun FunSpec.Builder.startMachete() = apply {
        appFeature.flattenFeatures().forEach {
            val className = it.coreClass.toClassName()
            addParameter(
                className.simpleName.asComponentDefinitionClassName().decapitalize(),
                className
            )
        }

        appFeature.flattenModules().forEach {
            val className = it.coreClass.toClassName()
            addParameter(
                className.simpleName.asModuleDefinitionClassName().decapitalize(),
                className
            )
        }

        createComponent(appFeature)
    }

    private fun FunSpec.Builder.createComponent(feature: Feature) = apply {
        val coreName = feature.coreClass.toClassName().simpleName
        val componentName = coreName.asComponentClassName()
        addStatement("""
            val ${componentName.decapitalize()} = ${componentName.decapitalize()}(
                ${coreName.asComponentDefinitionClassName().decapitalize()},
    ...
    private fun Feature.flattenFeatures(): Set<Feature> {
        return setOf(this) + features.map { it.flattenFeatures() }.flatten()
    }

    private fun Feature.flattenModules(): Set<Module> {
        return (modules.map { it.flattenModules() } + features.map { it.flattenModules() })
            .flatten()
            .toSet()
    }

    private fun Module.flattenModules(): Set<Module> {
        return setOf(this) + modules.map { it.flattenModules() }.flatten()
    }
...
//        appFeature.scopeDependencies.forEach {
...
//        val providers = appFeature.dependencies.map(::dependencyProvider)
...
//        val internalDependenciesProviders =
//            appFeature.internalDependencies.map(::dependencyProvider)
//        val featureFromChildProviders = appFeature.features.map(::featureFromChildProvider)
//
...
//                            val providers =
//                                appFeature.internalDependencies.map(::dependencyProvider)
//                                    .plus(appFeature.features.map(::featureFromChildProvider))
//
...
//        val providers = appFeature.scopeDependencies
//            .plus(appFeature.modules.map(Module::provideDependencies).flatten())
//            .distinct()
...
//                        .returns(componentClassName)
//                        .apply {
//                            val modulesWithDefinitionsNames = appFeature.modules.map {
//
...
//                            ${appFeature.internalDependencies
//                                    .map {
//                                        val providerName = it.providerName()
//                                        "$providerName = definition.$providerName"
//                                    }
//                                    .plus(appFeature.features.map(::featureFromChildProvider).map {
//                                        val name = it.first
//                                        "$name = definition.$name"
//                                    })
//                                    .plus(appFeature.dependencies.map {
//                                        val providerName = it.providerName()
